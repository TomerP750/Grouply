        ...
        <Menu>
            <main className="user-menu gap-5 bg-gradient-to-br from-white to-slate-100 dark:from-slate-800 dark:to-slate-900 flex flex-col items-start py-5 px-4 absolute top-7 mt-15 right-13 w-85 min-h-64 rounded-2xl shadow-2xl dark:text-gray-300 z-[1000]">
                ...
                <section className='flex flex-col w-full '>
                    ...
                    <NavLink to={`/profile/${user.id}`} className='inline-flex hover:underline justify-end text-sm self-end'>{t('userMenu.viewProfile')}</NavLink>
                </section>
                ...
                <div className="flex w-full font-medium dark:text-gray-300 justify-end items-center gap-2 px-2 py-1 ">
                    ...
                    <button onClick={toggle} className="text-sm cursor-pointer hover:underline">
                        {t("userMenu.theme")}: {theme === "dark" ? t("userMenu.themeDark") : t("userMenu.themeLight")}
                    </button>
                </div>
                ...
                <NavLink
                    to={"/about"}
                    className="flex w-full items-center gap-2 px-2 py-1 cursor-pointer hover:bg-gray-300/40 dark:hover:bg-gray-300/10"
                >
                    <HiOutlineQuestionMarkCircle size={20} />
                    <span className='text-sm'>{t("userMenu.helpAbout")}</span>
                </NavLink>
