...
export function NotificationMenu({ notifications }: NotificationMenuProps) {
    ...
    return (
        <Menu>
            <div className="z-1000 animate-[bounceUp_0.25s_ease-out_forwards] gap-4 bg-gradient-to-br from-white to-slate-100 dark:from-stone-900 dark:to-stone-900 flex flex-col items-stretch py-4 px-4 absolute top-2 mt-15 right-30 w-95 max-w-95 min-h-64 rounded-2xl shadow-2xl dark:text-gray-300">
                ...
                <header className="flex items-center justify-between mb-1">
                    ...
                    {hasNotifications && (
                        <button className="text-[11px] text-sky-600 dark:text-white/80 hover:underline">
                            Mark all as read
                        </button>
                    )}
                </header>
                ...
                {!hasNotifications ? (
                    <menu className="w-full flex flex-col items-center justify-center flex-1 gap-3 text-black/80 dark:text-white/80">
                        <BiBellMinus size={40} className="" />
